...
1. **逻辑学视角（“Knowing How”到“Knowing That”的逆向翻译）**
   Gilbert Ryle 区分了命题性知识（Knowing That）和程序性知识（Knowing How）。Skill 本质上是后者的载体——它是一种“直接做对”的非单调倾向。系统的工作，就是把隐性的 Knowing How 重新抽成一条条含有前置条件（IF）的 Knowing That，从而将其移交给机器代理执行。
...
4. **底层重构：“事件（Event）记录”与“产生式（Production）编译”的根本差异**
   传统的任务挖掘或 RPA 本质上只是“事件记录器”（例如录制屏幕点击的坐标）。此类手段只截取了认知决策的末端表现（缺乏依据），一旦环境或 UI 发生改变便瞬间失效。而在 ACT-R 理论框架下，真正的技能呈现为**产生式（Production）**：它不仅包含动作执行，更包含一套关联的 **左手边（触发目标与上下文判定前提）** 和 **右手边（基于判定实施的智能回应）** 的独立 IF-THEN 逻辑。系统的终极标的并非“复刻你怎么做”，而是提炼“你在何种认知逻辑下决定这么做”。唯有获得这套产生式程序，AI 才能像专业熟手般跨越表层像素并真正泛化复用。

**Skill Harvester 并不是在榨取员工知识来强化资本护城河。**
其核心机制是：通过捕捉人在系统前的“困难瞬间（预测误差峰值）”，温和地打断其机械流状态，以个人的数字副驾驶视角进行反事实询问，进而沉淀出属于个人专属特权且由产生式构筑的“认知外骨骼”。这一切，只为捍卫人的不可替代性。
...
3. **隐性知识**：我们引入反事实追问（Cognitive Task Analysis）的方式，在自然休息节点提问用户，把“当时怎么想的”剥离出来。
